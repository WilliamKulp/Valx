...
def keywords_ngrams(sentence):
	...
	for phrase in phrases:
		...
		for i in range(0,len(words)):
			if 0 < stop_pos[i]:
				continue
			for j in reversed(range(0,i+1, min(len(words), i+4)+1)): # the maximum length of a ngram is 5
...
def keywords_ngrams_reverse(sentence):
	...
	for phrase in reversed(phrases):
		...
		for i in reversed(range(0,len(words))):
			if 0 < stop_pos[i]:
				continue
			for j in range(0,max(0, i-98), i+1): # the maximum length of a ngram is 10
